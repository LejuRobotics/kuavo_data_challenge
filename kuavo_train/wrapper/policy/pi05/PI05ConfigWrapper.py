"""
PI05 Configuration Wrapper

This module provides a custom wrapper for PI05Config that adds support for depth image features
and RGB-Depth fusion using Cross-Attention method (方案二).
"""
...
class CustomPI05ConfigWrapper(PI05Config):
    """
    Custom PI05 Configuration Wrapper
    
    Extends PI05Config to support depth image features and RGB-Depth fusion.
    This wrapper allows passing depth-related parameters that are not part of
    the base PI05Config class.
    ...
    Attributes:
        custom: Dictionary for storing custom configuration parameters
            ...
            depth_fusion_method: Method for fusing RGB and depth features (only "cross_attention" for PI05)
            depth_fusion_dim: Dimension for depth fusion (None = auto)
            depth_backbone_init_from_vision_tower: Whether to initialize depth backbone from vision tower
    ...
    def __post_init__(self):
        ...
        default_map = {
            ...
            "STATE": NormalizationMode.QUANTILES,  # PI05 uses quantiles
            "ACTION": NormalizationMode.QUANTILES,  # PI05 uses quantiles
        }
        ...
        # Validate depth fusion method
        if self.use_depth and self.depth_fusion_method not in ["cross_attention", "sequence_concat"]:
            raise ValueError(
                f"Unsupported depth fusion method: {self.depth_fusion_method}. "
                f"Supported methods: 'cross_attention', 'sequence_concat'"
            )
    ...
    def from_pretrained(
        ...
        **policy_kwargs,
        ...
        Returns:
            Instance of CustomPI05ConfigWrapper loaded from pretrained config
        """
