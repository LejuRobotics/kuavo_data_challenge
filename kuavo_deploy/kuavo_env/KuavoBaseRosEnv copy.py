...
from kuavo_deploy.kuavo_env.lowpass_filter import LowPassFilter
...
class KuavoBaseRosEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # self.rate.sleep()
        # current_t = time.time()
        # time_to_sleep = max(0, current_t-t2)
        # if current_t < 
    ...
    def exec_action(self, action):
        ...
        if self.which_arm == 'both':
            ...
            # target_position = self.low_pass_filter.update(target_position)
        ...
        print(f'========= exec action time {toc - tic}')
        print(f'========== time control arm {t2 - t1}')
        print(f'========== time control eef {t3 - t2}')
