...
class ActionBuffer:
    ...
    简单的动作缓冲区，用于存储和获取动作。
    不使用RTC，只是简单的FIFO队列。
    多线程安全：Queue 本身线程安全，用 _lock 保护 actions_executed 及 clear 的语义。
    """
    def __init__(self, maxsize: int = 18):
        self.queue = Queue(maxsize=maxsize)
        self.maxsize = maxsize
        self._actions_executed = 0  # 记录已执行的动作数量，读写需加锁
        self._lock = Lock()
...
class ACTAsyncRealDemoConfig(HubMixin):
    ...
    min_buffer_size: int = 10  # Minimum buffer size before requesting new actions
    ...
    try:
        ...
        while not shutdown_event.is_set():
            ...
            if need_new_actions:
                ...
                # 推理开始时的已执行动作数，用于后续计算 delay 并丢弃 chunk 前部
                actions_executed_at_inference_start = action_buffer.get_actions_executed()
                logger.info(
                ...
                # 推理期间执行线程又执行了 delay 步，chunk 前 delay 步已过时，丢弃
                actions_executed_after = action_buffer.get_actions_executed()
                delay = actions_executed_after - actions_executed_at_inference_start
                drop = min(delay, chunk_size)
                if drop > 0:
                    postprocessed_actions = postprocessed_actions[drop:]
                
                # Add to buffer: 基于新观测的动作块应该替换旧队列（只放入未过时的后半段）
                action_buffer.put(postprocessed_actions[:18], replace=True)
                inference_step += 1
                
                # 计算总时间
                total_inference_time = time.time() - inference_start_time
                
                # 打印时间开销（含 delay / dropped）
                timing_msg = (
                    ...
                    f"chunk_size={chunk_size}, delay={delay}, dropped={drop}, "
                    f"put_size={postprocessed_actions.shape[0]}, buffer_size={action_buffer.qsize()}"
                )
